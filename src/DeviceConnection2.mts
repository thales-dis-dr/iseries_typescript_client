...
export class DeviceConnection2 {
  // the socket
  ...
  private send<T extends EmptyMessageWrapper>(message: T) {
    let i = message.i;
    let observable = this.messages.pipe(rx.filter((msg, index) => msg.i === i));
    this.ws.send(JSON.stringify(message));
    return observable;
  }

  open() {
    // TODO:  wait untill the ws socket is opened.
  ...
  sendCaptureData() {
    ...
    observable = observable.pipe(
      rx.filter((msg) => msg.t == ConnectionEvents.CAPTURE_DATA)
    );
